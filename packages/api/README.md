...
Key invariants enforced here: only agents push (`403 humans-do-not-push` at
the transport), token verification checks revocation (agents against
`token_hash`, users against `token_version`), secrets seal with libsodium and
never return in plaintext, and the boot refuses default JWT secrets in
production.
