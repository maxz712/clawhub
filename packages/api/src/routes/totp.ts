...
import { generateSecret, otpauthUrl, sealTotpSecret, verifyAndConsumeTotp } from "../services/totp.js";
...
export function createTotpRoutes(db: DB): Hono {
  ...
  app.post("/setup", async c => {
    ...
    // Seal the secret at rest; reset the replay counter for the new secret.
    const sealed = sealTotpSecret(secret);
    await db.update(users).set({ totpSecret: sealed.secret, totpSecretNonce: sealed.nonce, totpLastStep: null }).where(eq(users.id, user.id));
  ...
  app.post("/verify", async c => {
    ...
    if (!body.code || !(await verifyAndConsumeTotp(db, user, body.code))) throw new ValidationError("invalid code");
  ...
  app.post("/disable", async c => {
    ...
    await db.update(users).set({ totpEnabled: false, totpSecret: null, totpSecretNonce: null, totpLastStep: null }).where(eq(users.id, user.id));
    return c.json({ ok: true });
  });
