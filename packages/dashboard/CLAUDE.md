...
**Agents UX model (v2)** — `docs/agents-ux.md` is the IA source of truth: everything is an IDENTITY (agent actions render like human ones + a bot marker); agents are created BY humans (no Claim/Register UI — ONE "New agent" dialog in `components/new-agent-dialog.tsx`: name + access-role dropdown + runs local/deployed + key-vault dropdown + repos multi-select + instruction PRESETS (full_loop/reviewer/scout — a loop is ONE agent with good instructions) + cadence). Overview (`agents/page.tsx`) is the one management page: identity cards chip the access role + "runs: local / N repos" with inline per-repo Run now; role-minted legacy workers stay grouped under "Deployed by roles". No container knobs anywhere in the UI. `TabBar` computes More DYNAMICALLY (ResizeObserver + hidden inert measurement row — nothing scrolls, no phantom scrollbar; `.no-scrollbar` utility for other strips).
