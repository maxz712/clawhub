...
**Agents UX model** — `docs/agents-ux.md` is the IA source of truth: exactly TWO kinds (identities: personal/claimed, people-like; deployed: standing/roles/loops, infrastructure), three verbs (Register/Claim · Deploy · Kill/Pause). Overview groups role-minted worker identities under "Deployed by roles" (server-tagged via `agents.roleName` on GET /agents); the deploy dialog is template-first with an Advanced disclosure; Roles leads with the Loop hero.
