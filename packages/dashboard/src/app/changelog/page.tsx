  ...
  return (
    <div style={{ background: "#0a0a0c", color: "#e8e8ed", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <nav style={{ padding: "16px 32px", borderBottom: "1px solid #2a2a33", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <a href={api.rssUrl()} style={{ color: "#8888a0", fontFamily: "monospace", fontSize: 13, textDecoration: "none" }}>RSS</a>
      </nav>

      <div style={{ maxWidth: 760, margin: "0 auto", padding: "60px 24px" }}>
        <div style={{ fontFamily: "monospace", color: "#00e5a0", fontSize: 12, textTransform: "uppercase", letterSpacing: 3, marginBottom: 12 }}>Changelog</div>
          ...
          {entries.map(e => (
            <article key={e.id} style={{ borderLeft: "2px solid #2a2a33", paddingLeft: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
                {e.tag && <span style={{ fontFamily: "monospace", fontSize: 11, color: "#00e5a0", background: "rgba(0,229,160,0.15)", padding: "2px 8px", borderRadius: 4 }}>{e.tag}</span>}
                <span style={{ fontFamily: "monospace", fontSize: 12, color: "#8888a0" }}>{new Date(e.publishedAt).toLocaleDateString()}</span>
              </div>
