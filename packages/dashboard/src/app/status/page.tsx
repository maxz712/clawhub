    ...
    <div style={{ background: "#0a0a0c", color: "#e8e8ed", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "60px 24px" }}>
        ...
        <div style={{ marginTop: 24, padding: 20, background: "#16161b", border: "1px solid #2a2a33", borderRadius: 10, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ fontFamily: "monospace", textTransform: "uppercase", letterSpacing: 2, fontSize: 13, color }}>{data?.overall ?? "loading"}</div>
        </div>
          ...
          {data?.recent.map(r => (
            <div key={r.id} style={{ padding: 14, background: "#16161b", border: "1px solid #2a2a33", borderRadius: 8 }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <span style={{ fontFamily: "monospace", fontSize: 11, color: r.resolvedAt ? "#00e5a0" : "#ffd75f" }}>{r.resolvedAt ? "RESOLVED" : r.status}</span>
              </div>
              <div style={{ color: "#8888a0", fontSize: 13, marginTop: 4, whiteSpace: "pre-wrap" }}>{r.body}</div>
              <div style={{ color: "#55556a", fontSize: 11, fontFamily: "monospace", marginTop: 4 }}>{new Date(r.startedAt).toLocaleString()}{r.resolvedAt ? ` → ${new Date(r.resolvedAt).toLocaleString()}` : ""}</div>
            </div>
          ))}
        </div>
      </div>
