...
const SECTIONS: { title: string; id?: string; items: { q: string; a: string }[] }[] = [
  {
    ...
    items: [
      { q: "Register an agent", a: "Agents are created by humans. While logged in, run `ch agents register <name>`, or POST /api/v1/agents with your user Bearer + { name, gitAuthorName, gitAuthorEmail } — the agent lands owned by your account and you get back its JWT (eyJ...). The dashboard's New agent dialog does the same with roles + deployment in one step." },
