  ...
  return (
    <div style={{ background: "#0a0a0c", color: "#e8e8ed", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <nav style={{ padding: "16px 32px", borderBottom: "1px solid #2a2a33", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <Link href="/trending" style={{ color: "#8888a0", fontFamily: "monospace", fontSize: 13, textDecoration: "none" }}>Trending →</Link>
      </nav>

      <div style={{ maxWidth: 960, margin: "0 auto", padding: "60px 24px" }}>
        <div style={{ fontFamily: "monospace", color: "#00e5a0", fontSize: 12, textTransform: "uppercase", letterSpacing: 3, marginBottom: 12 }}>Leaderboard</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {rows.map(r => (
            <Link key={r.id} href={`/u/${r.name}`} style={{ display: "grid", gridTemplateColumns: "56px 1fr auto auto auto", alignItems: "center", gap: 20, padding: "14px 18px", background: "#16161b", border: "1px solid #2a2a33", borderRadius: 10, textDecoration: "none", color: "#e8e8ed" }}>
              <span style={{ fontFamily: "monospace", fontWeight: 700, color: r.rank <= 3 ? "#ffd75f" : "#8888a0" }}>#{r.rank}</span>
              <span style={{ fontFamily: "monospace", fontWeight: 600 }}>@{r.name}</span>
              <span style={{ fontFamily: "monospace", fontSize: 13, color: "#5f9eff" }}>{r.changesMerged} merged</span>
              <span style={{ fontFamily: "monospace", fontSize: 13, color: "#00e5a0" }}>{r.changesOpened} opened</span>
              <span style={{ fontFamily: "monospace", fontSize: 13, color: "#ffd75f" }}>{r.reviewsSubmitted} reviews</span>
            </Link>
          ))}
        </div>
      </div>
