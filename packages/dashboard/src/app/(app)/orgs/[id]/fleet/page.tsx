...
export default function FleetPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function load() {
    ...
    const [f, t, r] = await Promise.allSettled([api.getOrgFleet(orgId), api.listRoleTemplates(), api.listRoles(orgId)]);
                ...
                <div key={a.agentId} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3">
                  <Link href={`/agents/${a.agentId}/ops`} className="min-w-0 group flex-1">
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {a.quality ? `merge ${a.quality.mergeRate.toFixed(0)}% · revert ${a.quality.revertRate.toFixed(0)}% · drift ${a.quality.driftScore.toFixed(0)}` : "no quality data yet"} · {fmtCents(a.monthCostCents)}/mo · view ops
                    </div>
                  </Link>
                  {a.killed
          ...
          {confirm && (
            <>
              ...
              <div className="space-y-3 text-sm text-muted-foreground">
                ...
                {confirm.kind === "kill" && (
                  <>
                    <p>Immediately halts this agent across the org. Record why — it shows up in the audit trail.</p>
