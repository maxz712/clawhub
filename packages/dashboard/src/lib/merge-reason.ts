 ...
 * USER-namespace repo where the supervising human owns and reviews their own
 * agent's work) or whether an INDEPENDENT approver is required (an org repo, or
 * a teammate reviewing a colleague's change). `solo` is true only when both:
 ...
 * "self-approving your own agent's work is expected" framing; an org repo or a
 * teammate gets the stricter "needs an independent human reviewer" framing.
 */
export function humanizeMergeReason(reason: MergeReason | undefined, ctx?: MergeReasonContext): string {
  ...
  switch (reason) {
    case "needs_human_approval":
      return solo
        ? "Submit an Approve review below to unblock — self-approving your own agent's work is expected for solo repos."
        : "This change needs an approving CODE review from a human other than the author.";
    case "needs_more_approvals":
      return solo
        ? "More approvals required — submit an Approve review below to unblock. Self-approving your own agent's work is expected for solo repos."
