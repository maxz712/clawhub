...
- **Every non-default-branch push opens a Change** (the PR equivalent). Commit trailers — `Intent:`, `Risk:`, `Scope:`, `Review-Focus:`, `Closes:`, `Agent:` — drive the review UI (the `ch` CLI + MCP compose them for you).
- **Inference informs, determinism decides.** The merge gate, the risk engine, and verification attestations are 100% deterministic — no LLM's opinion can move them. Inference enters only the *focus and evidence* layer: even a trailer-less push gets a deterministic Review Brief synthesized from the diff, and an optional platform-keyed **advisory** reviewer can add a capped, precision-focused opinion (advisory-only — it never gates). BYO-key and self-host keep inference entirely under your control.
- **Focused review is the default.** Humans see only the lines that matter — flagged via `Review-Focus:` trailers, `// REVIEW:` comments, reviewer agents, or the deterministic Review Brief; the full diff is one click away.
