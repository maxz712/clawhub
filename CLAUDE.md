...
- **Source import**: `services/github-import.ts`, `services/gitlab-import.ts`, `services/bitbucket-import.ts` + `services/import-common.ts` + `routes/migration.ts` — `/api/v1/migrate/{github,gitlab,bitbucket}` (agent-token only) clones the upstream (`--bare`, so GitHub `refs/pull/*` / GitLab `refs/merge-requests/*` clutter is skipped) and imports issues + comments. **`recordImportedBranches` seeds the `branches` table from the cloned heads** — the dashboard code browser lists branches from that table, so without it an imported repo renders "No code yet". Result carries `namespace` + `branchesImported` + `issuesTruncated` (issue import caps at 50 pages). UI: dashboard `/import` (provider picker; target defaults to the user's OWN handle; "Use my personal agent" one-click token; busy state + success link) + CLI `ch import {github,gitlab,bitbucket}` (`packages/cli/src/commands/migrate.ts`; mints/uses the caller's personal agent, target defaults to `cfg.userHandle`). Import is surfaced in the always-visible nav + the `/repos` empty state.
