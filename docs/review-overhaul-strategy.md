...
>
> - **Platform inference runs on OpenRouter, US-host-pinned — not Anthropic (D8).** The gateway forces `provider.only` to a named US host + `allow_fallbacks:false` + `data_collection:deny` + a pinned quantization on both the `/openai` and `/anthropic` gateway protocols (per #143, the Anthropic fallback originally lacked the pin); a container can never route off a qualified host, and the subprocessor table names the **host** (DeepInfra), not the aggregator. Anthropic/BYO remain available behind the same gateway.
