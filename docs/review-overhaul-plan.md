...
- **Custody rule (ground rule 1) holds, but on OpenRouter, not Anthropic (D8).** The native reviewer/verifier are `keySource='platform'` and reach OpenRouter through `OPENAI_BASE_URL=<api>/api/v1/llm/openai/v1` with a per-run gateway token; the platform key never enters a container. The gateway FORCES `provider.only` to a named US host + `allow_fallbacks:false` + `data_collection:deny` + a pinned quant, and rejects any model outside the qualified catalog on both the `/openai` and `/anthropic` gateway protocols via the shared `openRouterAdmission` gate (fixing #143: the Anthropic fallback had lacked the catalog + allowlist enforcement). `services/llm-catalog.ts` + `routes/llm-gateway.ts` + `services/platform-quota.ts`.
...
- **Q4 (§6) landed early — as of 2026-07-05 every code-buildable N-item is built:** N2 GitHub App mirror-and-verify (`services/github-app.ts` + `github-mirror.ts`, migration 0047); N3 multi-model catalog GA — `/llm/openai/*` gateway, `MODEL_CATALOG`, org-connected keys (`services/org-llm-key.ts`), the public catalog endpoint (`GET /api/v1/public/llm-catalog`), the repo review-model pin (settings selector → the system reviewer row, catalog-validated), the org provider allowlist (`organizations.llm_provider_allowlist`, migration 0056, gateway-enforced), and the verify-bench (`packages/api/scripts/verify-bench.ts` + fixtures); N4 visual regression (`clawhub-visual-diff`, baseline store, EvidencePanel base/head/diff triptych on the `visual:base|head|diff` label convention); N5 Loop v2 — platform-key Loops (`installLoop keySource:'platform'`, zero-setup, mandatory budget row at install), config/migration claim validation (evidence-required, migration needs services/dind), deterministic issue routing, the server-authored-Change primitive, and the LIVE demo endpoint (`/api/v1/public/demo/*`, template-constrained + fail-closed caps, dark until `CLAWHUB_DEMO_REPO` — per the plan's own soak guardrail); N7 quota cache + gateway runbook. **Still open by their own gates, not by omission:** N1 (the quarter-start re-planning ritual, runs Oct 1), N6 (conditional on discovery saying the enterprise buyer exists), and §7's 2027–28 blocks (decision gates on business metrics, not buildable code).
...
- **D8 · Open-model routing (2026-07-03):** platform-keyed open-model traffic goes through **OpenRouter pinned to a named US-jurisdiction host** (Exacto/explicit provider order, fallbacks limited to hosts also qualified), never through PRC first-party APIs (DeepSeek/Z.ai/Moonshot direct) — the subprocessor story outranks the price delta (DeepSeek V4 Pro mid verify run ≈ $0.10 direct vs ≈ $0.15 pinned-US vs $1.06 Sonnet; a nickel against a $2.00 SKU is not worth the trust story). The pin is enforced on both `/openai` and `/anthropic` gateway routes via the shared `openRouterAdmission` gate, closing #143 where the Anthropic fallback reached OpenRouter unguarded and exposed default Loops to uncatalogued models and billing mismatches. **Qualification is per (model, host, quantization)** — hosts serve different quants (e.g. FP4 on some public endpoints), so the catalog entry pins exactly what the bench qualified, and the subprocessor table names the host, not the aggregator. Account data policy excludes providers that train on inputs. Graduation at volume = direct to the pinned US host (negotiated rates), not to the model vendor. BYO remains anything the user configures, including PRC-direct — their key, their data decision. Refines D7: Z.ai direct = BYO recipe only.
