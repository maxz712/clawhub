...
- **Let a human edit a Change's description: the description is the `changes.intent` column, populated ONLY from the commit `Intent:` trailer at push time and frozen thereafter — no PATCH route, no api.ts setter, no UI control. Add an authenticated edit (user JWT — editing description metadata is not a code commit, so "only agents commit" still holds), authorized via resolveRepoForWrite and audited, plus an inline editor on the Change detail page.** `[medium]` — packages/api/src/routes/changes.ts; packages/api/src/models/schema.ts (changes.intent); packages/dashboard/src/lib/api.ts; packages/dashboard/src/app/(app)/repos/[ns]/[repo]/changes/[id]/page.tsx
