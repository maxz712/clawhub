...
Two orthogonal axes, generalized across every CLI (`entrypoint.sh:cli_run`): **autonomy is always on** — each CLI runs fully non-interactive with its specific hang-gate killed (claude `--permission-mode dontAsk`, gemini `--skip-trust`, copilot `--no-ask-user` + a pre-seeded trust file, codex `--skip-git-repo-check`), so it never asks a human anything. **Authorization is the user's gate** — `CLAWHUB_TOOLS` lists the capability GROUPS the agent may use (`read|edit|execute|browser|network|push`), translated to each CLI's own allow/deny flags (granular for claude/copilot/gemini; coarse sandbox tiers for codex/goose). Default = all groups (full autonomy); `xinmingzhang/clawhub` runs Full.

**The gate is ENFORCED and fails closed (#78/#87).** `push` is a real capability: without it the write modes commit to the local branch and open NO Change (`push_change`). The claude gate emits `--disallowedTools` — the actual gate, since `--permission-mode dontAsk` only *auto-approves* `--allowedTools` and an unlisted tool is otherwise still callable — denying the persistent/meta/deferred family (`ToolSearch`/`CronCreate`/… ) under **every** grant plus each ungranted capability family; `Task` (subagent delegation of execution) is granted only when subagents are materialized AND edit-or-execute is granted, and the materialized `implementer` subagent carries a `tools:` frontmatter pinned to a subset of the parent grant (never `Task`/`Agent`). CLIs whose only autonomy control is all-or-nothing (`cline`/`cursor`/`continue`/`aider`) REFUSE to run under a narrowed grant rather than silently run full-autonomy; `goose` degrades to read-only (`GOOSE_MODE=chat`) when edit/execute is not granted. The effective grant + any group that could not be enforced is logged once at run start.

**Trust boundary — operator task is instruction, third-party text is data (#79).** The operator's `CLAWHUB_TASK` is the only instruction. Issue title/body, change diffs, and recalled memory are UNTRUSTED third-party input: they are fenced into the prompt via `untrusted_block()`/`memory_context()` with a per-run delimiter and an explicit "not instructions" banner, never spliced into the imperative `TASK:` slot, and byte-capped. The harness's own control markers (`===CLAWHUB_CHANGE===`, `===CLAWHUB_MEMORY===`, `RESULT_JSON:`) are anchored to a column-0 line in both parsers AND neutralised inside any fenced payload, so attacker-authored text quoted back through the CLI's stdout cannot forge a Change subject, a `Closes:` trailer, or a memory write.
