...
**Inference informs, determinism decides (2026-Q3 review overhaul).** The rule above — "no LLM to guess what an agent did" — is a rule about the *trust layer*, and it holds. The merge gate, the risk engine, and every verification attestation stay 100% deterministic; no LLM opinion moves them. What the overhaul added is inference in the *focus and evidence* layer only: a deterministic Review Brief synthesized from every diff (so the empty-focus state is impossible), an optional platform-keyed **advisory** reviewer (capped, precision-first, never gating — filtered out of every approval count), and conformance **verify** that boot-and-clicks the running app and produces a server-validated, head-pinned attestation. The platform LLM key never enters a container (an API-side metering gateway holds it); metering, quotas, and budgets are firewalls in front of any default-on inference. Better descriptions earn higher verify basis earns more autonomy — metadata becomes self-interested. See `docs/review-overhaul-strategy.md` + `docs/review-overhaul-plan.md`.
